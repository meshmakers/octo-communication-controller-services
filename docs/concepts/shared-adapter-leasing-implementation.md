...
And the two properties the first four left out:

- **`Scale_MovesItsOwnReleaseAndLeavesAnotherTenantsPoolWhereItWas`** — the scale verb and the
  owner stamp both select on `app.kubernetes.io/instance={release}` and act on everything returned.
  The pool namespace holds many tenants' pools by design, so a selector matching too broadly
  resizes — or takes ownership of, and later collects — an unrelated tenant's pool. A second
...
| `Services/TriggerManagementService.UpdateScheduleAsync` + `Consumers/LeaseTriggerConsumer` | **AB#5278**: a cron trigger of a pipeline on a `Leased` adapter is scheduled onto the controller's durable `octo::com-controller::lease-trigger` queue (`LeaseTriggerMessage`) instead of the adapter's per-pipeline queue; the consumer turns every tick into a queued work item through `StartExecutePipelineAsync`, coalescing a tick whose previous run is still `Queued`. No adapter-side schedule and no co-wake for such pipelines. |
| `Repository/QueuedCaller` + `Services/JwtExpiry` + `LeaseService.ResolveCallerAccessToken` | **AB#5279**: the invoker of a queued work item is persisted on the `PipelineExecution` (seven optional runtime-state attributes, model 4.2.0), projected onto `QueuedExecution` / `LeaseRequest` / `LeaseDto`, token encrypted at rest and dropped at grant when expired; the mesh adapter's `LeaseCallerCarryThrough` applies it under the pipeline's `CallerBinding` rule |
| `Services/WorkloadOnDemandCapabilityService.EvaluateForLeaseAsync` | **AB#5278**: the `Leased` gate — process-bound triggers plus the adapter-bound ones that cannot produce a lease (`FromHttpRequest` → AB#5258, `FromPipelineDataEvent` → AB#5231); used by `DeploymentSiteService` (workload deploy) and `AdapterService` (pipeline deploy) |
